...
echo "=== [2/7] Install Python 3.13 via deadsnakes PPA ==="
...
echo "=== [3/7] Clone repo ==="
cd /root
git clone https://github.com/Vanz23-23/Synk.git synk
cd synk

echo "=== [4/7] Create virtualenv and install dependencies ==="
echo "    (torch is ~2 GB — this takes 5-10 minutes)"
python3.13 -m venv .venv
.venv/bin/pip install --upgrade pip
.venv/bin/pip install -r requirements.txt

echo "=== [5/7] Create logs directory ==="
mkdir -p logs
...
echo "=== [7/7] Install systemd units and start services ==="
...
systemctl enable synk-bot.service synk-watchdog.timer
systemctl start synk-bot.service synk-watchdog.timer
